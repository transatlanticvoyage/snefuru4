...
export default function Tebnar1() {
  ...
  const handlePopupOpen = () => {
    ...
    if (!hasTabInUrl) {
      ...
      const lastTab = localStorage.getItem('tebnar1_lastActiveTab') || 'ptab1';
  ...
  const handleTabChange = (tab: string) => {
    ...
    localStorage.setItem('tebnar1_lastActiveTab', tab); // Remember this tab for next time
    updatePopupURL(true, tab);
  };

  // SOPTION1 and SOPTION2 handler - adapted for tebnar1
  ...
  const handleSelectedItemsFunction = async (functionName: string) => {
    ...
    if (kz101Checked) {
      ...
      itemsToProcess = currentItems.map(plan => plan.id);
    } else {
  ...
  useEffect(() => {
    document.title = '/tebnar1 - Snefuru';
  ...
  return (
    <div className="w-full mx-auto">
      ...
      <div className="w-full px-4">
        ...
        {/* Functions Popup Button - positioned between Submit func_create_plans_make_images_1 and uitablegrid21 */}
        <div className="mb-4">
          <button
            ...
            <div className="bg-white w-full h-full max-w-[95vw] max-h-[95vh] rounded-lg shadow-xl relative overflow-hidden">
              ...
              >
                ...
                <span className="font-mono text-sm overflow-hidden whitespace-nowrap text-ellipsis flex-1">
                  {(() => {
                    // Parse URL to highlight batchid parameter for tebnar1
                    try {
              ...
              >
                ...
                >
                  ...
                  />
                  SOPTION2 - Select All Items In Current Pagination | {currentItems.length} rows
                </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                          <button
                            onClick={() => handleSelectedItemsFunction('tbn1_example_function_1')}
                          ...
                          >
                            {functionLoading ? 'Processing...' : 'Example Function 1 - Process Selected Plans'}
                          </button>
                          
                          <button
                            onClick={() => handleSelectedItemsFunction('tbn1_example_function_2')}
                            disabled={functionLoading || (!kz101Checked && !kz103Checked) || (kz101Checked && selectedRows.size === 0)}
                            className={`px-4 py-2 rounded font-medium ${
                              functionLoading || (!kz101Checked && !kz103Checked) || (kz101Checked && selectedRows.size === 0)
                                ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                          ...
                          <button
                            onClick={() => handleSelectedItemsFunction('tbn1_regenerate_images')}
                            disabled={functionLoading || (!kz101Checked && !kz103Checked) || (kz101Checked && selectedRows.size === 0)}
                            className={`px-4 py-2 rounded font-medium ${
                              functionLoading || (!kz101Checked && !kz103Checked) || (kz101Checked && selectedRows.size === 0)
                                ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                          ...
                          >
                            {functionLoading ? 'Processing...' : 'Regenerate Images for Selected Plans'}
                          </button>
                        </div>
                        ...
                        {kz103Checked && (
                          <p className="text-sm text-gray-600">
                            SOPTION2 selected: All {currentItems.length} items will be processed
                          </p>
                        )}
                        ...
                        <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                          ...
                          <div className="text-xs text-gray-600">
                            • Current Page Items: {currentItems.length}
                          </div>
                          <div className="text-xs text-gray-600">
                            • Total Filtered Plans: {filteredPlans.length}
                          </div>
                          {selectedBatchId && (
                            <div className="text-xs text-gray-600">
                              • Active Batch Filter: {selectedBatchId}
                            </div>
                          )}
                        </div>
                      </div>
