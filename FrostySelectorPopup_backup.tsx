...
export default function FrostySelectorPopup({ isOpen, onClose, pageType }: Props) {
        ...
        {/* Two Column Layout - Full Height */}
          ...
          <div className="w-96 bg-gray-50 flex flex-col">
            <div className="p-6 flex-1 overflow-auto">
              <div className="mb-6">
                <h3 className="text-xl font-semibold text-gray-800 mb-3">Selection Status</h3>
          });
  ...
  const processBaobobTransform = async (
    ...
    try {
      ...
      // Step 2: Insert transformed records in bulk (RESTORED ORIGINAL APPROACH + baobab_attempt_id)
      const batchInsertSize = 5000; // Larger batches for maximum speed
      let groupNum = 0;
      const recordsToInsert: any[] = [];
      const groupToGlobalRowIds: Map<string, number[]> = new Map(); // Track relations for bulk insert
      let batchTransformedRows = 0;
      let batchInsertedRows = 0;
      ...
      for (const [keyStr, groupData] of globalGroups) {
        ...
        // Create aggregated zip codes as STRING (original format)
        const uniqueZipCodes = [...new Set(groupData.zip_codes)].sort();
        const aggregatedZipCodes = uniqueZipCodes.join(','); // STRING format like original
        
        // Prepare transformed data (original structure + baobab_attempt_id)
        const transformedData = {
          ...
          aggregated_zip_codes: aggregatedZipCodes, // STRING format like original
          payout: parseFloat(groupKey.payout),
          baobab_attempt_id: attemptId, // NEW: Keep baobab_attempt_id feature
        ...
        // Store relation mapping for bulk relations insert
        groupToGlobalRowIds.set(JSON.stringify({
          jrel_release_id: groupKey.rel_release_id,
          jrel_subsheet_id: groupKey.rel_subsheet_id,
          jrel_subpart_id: groupKey.rel_subpart_id,
          city_name: groupKey.city_name,
          state_code: groupKey.state_code,
          payout: parseFloat(groupKey.payout)
        }), groupData.global_row_ids);
        
        // Insert in batches (ORIGINAL BULK APPROACH)
        if (recordsToInsert.length >= batchInsertSize || groupNum === totalGroups) {
          ...
          // Step 2A: Bulk insert to leadsmart_transformed (ORIGINAL FAST APPROACH)
          const { data: insertedRecords, error: insertError } = await supabase
            .from('leadsmart_transformed')
            .insert(recordsToInsert)
            .select('mundial_id, jrel_release_id, jrel_subsheet_id, jrel_subpart_id, city_name, state_code, payout');
          
          if (insertError) {
            console.error('Bulk insert error:', insertError);
            // If bulk insert fails, skip this batch but continue (original behavior)
          } else {
            insertedRows += recordsToInsert.length;
            
            // Step 2B: Bulk insert relations (NEW EFFICIENT ADDITION)
            if (insertedRecords && insertedRecords.length > 0) {
              const relationRecords: any[] = [];
              
              // Match inserted records to their original global_row_ids
              insertedRecords.forEach((insertedRecord) => {
                const lookupKey = JSON.stringify({
                  jrel_release_id: insertedRecord.jrel_release_id,
                  jrel_subsheet_id: insertedRecord.jrel_subsheet_id,
                  jrel_subpart_id: insertedRecord.jrel_subpart_id,
                  city_name: insertedRecord.city_name,
                  state_code: insertedRecord.state_code,
                  payout: insertedRecord.payout
                });
                
                const globalRowIds = groupToGlobalRowIds.get(lookupKey);
                if (globalRowIds) {
                  globalRowIds.forEach((globalRowId) => {
                    relationRecords.push({
                      original_global_id: globalRowId,
                      transformed_mundial_id: insertedRecord.mundial_id,
                      baobab_attempt_id: attemptId // NEW: Include baobab_attempt_id in relations
                    });
                  });
                }
              });
              
              // Bulk insert relation records
          ...
          batchTransformedRows += recordsToInsert.length;
          recordsToInsert.length = 0; // Clear the batch
          
          // Update progress (original approach)
          ...
          await supabase
            ...
            .update({
              ...
              rows_processed: processedRows,
              rows_transformed: batchTransformedRows,
              rows_inserted: batchInsertedRows,
              rows_updated: batchUpdatedRows,
              operations_per_second: opsPerSecond,
              current_phase: `Bulk inserting: ${groupNum.toLocaleString()}/${totalGroups.toLocaleString()}` // Original message
            })
            .eq('baobab_attempt_id', attemptId);
        }
      }
