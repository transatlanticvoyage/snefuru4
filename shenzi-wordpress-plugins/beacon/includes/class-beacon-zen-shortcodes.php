...
class Beacon_Zen_Shortcodes {
    ...
    /**
     * Maybe register shortcodes based on Beacon settings
     */
    public function maybe_register_shortcodes() {
        $mode = get_option('beacon_shortcode_mode', 'automatic');
        
        switch ($mode) {
            case 'automatic':
                // Default: Only if BOTH Snefuruplin AND Grove not active
    ...
    public function enqueue_styles() {
        wp_enqueue_style(
            'beacon-zen-shortcodes',
            BEACON_PLUGIN_URL . 'assets/zen-shortcodes.css',
            array(),
            BEACON_PLUGIN_VERSION
        );
    }
    ...
    public function render_services($atts) {
        ...
        $services = Beacon_Database::get_services(array(
    ...
    public function render_single_service($atts) {
        ...
        $service = Beacon_Database::get_service(intval($atts['service_id']));
    ...
    public function render_locations($atts) {
        ...
        $locations = Beacon_Database::get_locations(array(
            'limit' => intval($atts['limit']),
            'pinned_first' => $atts['pinned_first'] === 'true',
            'orderby' => $atts['orderby'],
            'order' => strtoupper($atts['order'])
    ...
    public function render_single_location($atts) {
        ...
        $location = Beacon_Database::get_location(intval($atts['location_id']));
    ...
    public function render_sitespren($atts) {
        ...
        $sitespren = Beacon_Database::get_sitespren(intval($atts['wppma_id']));
