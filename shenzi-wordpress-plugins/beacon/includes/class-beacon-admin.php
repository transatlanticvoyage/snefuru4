...
class Beacon_Admin {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_beacon_driggs_export', array($this, 'beacon_driggs_export'));
        add_action('wp_ajax_beacon_driggs_save_all', array($this, 'beacon_driggs_save_all'));
        add_action('wp_ajax_beacon_driggs_reset', array($this, 'beacon_driggs_reset'));
        add_action('wp_ajax_beacon_update_site_settings', array($this, 'beacon_update_site_settings'));
    }
    ...
    public function add_admin_menu() {
        ...
        add_submenu_page(
            ...
            'manage_options',
            'beacon_driggs_mar',
            array($this, 'beacon_driggs_mar')
        );
        
        add_submenu_page(
            'beaconhub',
            'Beacon Locations Mar',
            'Beacon Locations Mar',
            'manage_options',
            'beacon_locations_mar',
            array($this, 'beacon_locations_mar')
        );
        ...
        add_submenu_page(
            ...
            'manage_options',
            'beacon_services_mar',
            array($this, 'beacon_services_mar')
        );
    }
    ...
    public function beaconhub_page() {
        ...
        wp_redirect(admin_url('admin.php?page=beacon_driggs_mar'));
        exit;
    }
    
    /**
     * Beacon Driggs Mar page - Main site management interface
     */
    public function beacon_driggs_mar() {
        // AGGRESSIVE NOTICE SUPPRESSION - Remove ALL WordPress admin notices
        ...
        <div class="wrap" style="margin: 0; padding: 0;">
            ...
            <div style="padding: 20px;">
                ...
                <div style="background: #f9f9f9; border: 1px solid #ddd; padding: 12px 20px; margin-bottom: 20px; border-radius: 5px;">
                    <form id="beacon-site-settings-form">
                        <div style="display: flex; align-items: center; gap: 25px; flex-wrap: wrap;">
                            <div style="display: flex; align-items: center; gap: 8px;">
                                <label for="beacon-site-title" style="font-weight: 600; color: #333;">Site Title:</label>
                                <input type="text" id="beacon-site-title" name="blogname" value="<?php echo esc_attr(get_option('blogname')); ?>" style="padding: 6px 10px; border: 1px solid #ddd; border-radius: 3px; font-size: 13px; width: 200px;">
                            </div>
                            
                            <div style="display: flex; align-items: center; gap: 8px;">
                                <label for="beacon-site-tagline" style="font-weight: 600; color: #333;">Tagline:</label>
                                <input type="text" id="beacon-site-tagline" name="blogdescription" value="<?php echo esc_attr(get_option('blogdescription')); ?>" style="padding: 6px 10px; border: 1px solid #ddd; border-radius: 3px; font-size: 13px; width: 250px;">
                            </div>
                            
                            <div style="display: flex; align-items: center; gap: 8px;">
                                <label for="beacon-site-icon" style="font-weight: 600; color: #333;">Site Icon:</label>
                                <div style="display: flex; align-items: center; gap: 8px;">
                                    <?php 
                                    ...
                                    ?>
                                    <button type="button" id="beacon-choose-site-icon" class="button button-small">Choose Icon</button>
                                    <input type="hidden" id="beacon-site-icon-id" name="site_icon" value="<?php echo esc_attr($site_icon); ?>">
                                </div>
                            </div>
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px;">
                        ...
                        <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                            ...
                            <div style="display: flex; gap: 5px;">
                                <button id="beacon-export-sharkintax" class="button button-secondary button-small" data-format="sharkintax" title="Export as Sharkintax format">sharkintax</button>
                                <button id="beacon-export-walrustax" class="button button-secondary button-small" data-format="walrustax" title="Export as Walrustax format">walrustax</button>
                                <button id="beacon-export-xls" class="button button-secondary button-small" data-format="xls" title="Export as Excel format">xls</button>
                                <button id="beacon-export-csv" class="button button-secondary button-small" data-format="csv" title="Export as CSV format">csv</button>
                                <button id="beacon-export-sql" class="button button-secondary button-small" data-format="sql" title="Export as SQL format">sql</button>
                            </div>
                            <div style="border-left: 1px solid #ddd; padding-left: 10px;">
                                <button id="beacon-save-all-btn" class="button button-primary">Save All Changes</button>
                                <button id="beacon-reset-btn" class="button button-secondary">Reset to Defaults</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div style="background: white; border: 1px solid #ddd; border-radius: 5px; overflow: hidden;">
                    <div style="overflow-x: auto;">
                        <table id="beacon-driggs-table" style="width: auto; border-collapse: collapse; font-size: 14px;">
                            <thead style="background: #f8f9fa;">
                                <tr>
                                    <th style="padding: 12px 8px; border: 1px solid #ddd; font-weight: bold; text-align: left; background: #f0f0f0; width: 50px;">
                                        <input type="checkbox" id="beacon-select-all" style="width: 20px; height: 20px;">
                                    </th>
        ...
        .beacon-toggle-switch input[type="checkbox"]:focus + .beacon-toggle-slider {
        ...
        #beacon-driggs-table td {
            vertical-align: middle;
        }
        
        #beacon-driggs-table td[data-field] {
            min-height: 30px;
        }
        
        #beacon-driggs-table textarea,
        #beacon-driggs-table input[type="text"],
        #beacon-driggs-table input[type="email"],
        #beacon-driggs-table input[type="number"] {
            font-family: inherit;
            font-size: 14px;
        }
        ...
        .beacon_roaring_div:hover {
            background-color: #e0e0e0;
            border-color: #666;
        }
        
        /* Ensure stuff3 column cells have no padding */
        #beacon-driggs-table td.beacon-stuff3-cell {
            padding: 0 !important;
            width: 20px !important;
        }
        
        /* Read-only field styling */
        .beacon-driggs-readonly-field {
            color: #666 !important;
        ...
        jQuery(document).ready(function($) {
            ...
            // Load initial data
            loadBeaconDriggsData();
            
            function loadBeaconDriggsData() {
                $.ajax({
                    url: ajaxurl,
                    type: 'POST',
                    data: {
            ...
            function displayData() {
                ...
                const fields = [
                    ...
                    // Additional Beacon-specific fields not in /drom
                ...
                fields.forEach(function(field) {
                    ...
                    if (field.type === 'boolean') {
                        ...
                        valueTd.addClass('beacon-driggs-readonly-field');
                    ...
                    let stuff3Td = $('<td class="beacon-stuff3-cell" style="padding: 0; border: 1px solid #ddd; width: 20px;"></td>');
                    if (isSpecialBg) stuff3Td.css('background-color', '#d5d5d5');
                    let roaringDiv = $('<div class="beacon_roaring_div">📡</div>');
            ...
            function createToggleSwitch(fieldKey, isChecked) {
                let switchContainer = $('<div class="beacon-toggle-switch" style="position: relative; display: inline-block; width: 60px; height: 34px;"></div>');
                
                let checkbox = $('<input type="checkbox" style="opacity: 0; width: 0; height: 0;">');
                checkbox.prop('checked', isChecked);
                checkbox.attr('data-field', fieldKey);
                
                let slider = $('<span class="beacon-toggle-slider" style="position: absolute; cursor: pointer; top: 0; left: 0; right: 0; bottom: 0; background-color: #ccc; transition: .4s; border-radius: 34px;"></span>');
                
                let knob = $('<span class="beacon-toggle-knob" style="position: absolute; content: \'\'; height: 26px; width: 26px; left: 4px; bottom: 4px; background-color: white; transition: .4s; border-radius: 50%;"></span>');
            ...
            function updateField(field, value) {
                $.ajax({
                    ...
                    type: 'POST',
                    data: {
                        action: 'beacon_driggs_update_field',
                        nonce: '<?php echo wp_create_nonce('beacon_driggs_nonce'); ?>',
                        field: field,
                        value: value
                    },
            ...
            $('#beacon-site-settings-form').on('submit', function(e) {
                e.preventDefault();
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data: {
                        action: 'beacon_update_site_settings',
                        nonce: '<?php echo wp_create_nonce('beacon_driggs_nonce'); ?>',
            ...
            // Site icon chooser
            $('#beacon-choose-site-icon').on('click', function(e) {
                e.preventDefault();
    ...
    /**
     * Beacon Locations Mar page
     */
    public function beacon_locations_mar() {
        $this->suppress_all_admin_notices();
        
        ?>
        <div class="wrap">
            <h1>🚨 Beacon Hub - Locations Manager</h1>
    ...
    /**
     * Beacon Services Mar page  
     */
    public function beacon_services_mar() {
        $this->suppress_all_admin_notices();
        
        ?>
        <div class="wrap">
            <h1>🚨 Beacon Hub - Services Manager</h1>
     ...
     */
    public function beacon_driggs_get_data() {
        check_ajax_referer('beacon_driggs_nonce', 'nonce');
     ...
     */
    public function beacon_driggs_update_field() {
        check_ajax_referer('beacon_driggs_nonce', 'nonce');
     ...
     */
    public function beacon_driggs_export() {
        check_ajax_referer('beacon_driggs_nonce', 'nonce');
        ...
        // Validate format
        if (!Beacon_Tax_Exports::is_valid_format($format)) {
        ...
        try {
            // Generate filename
            $filename = Beacon_Tax_Exports::generate_export_filename($format);
            ...
            switch ($format) {
                case 'sharkintax':
                    $content = Beacon_Tax_Exports::generate_sharkintax($data, $fields);
                    break;
                case 'walrustax':
                    $content = Beacon_Tax_Exports::generate_walrustax($data, $fields);
                    break;
                case 'csv':
                    $content = Beacon_Tax_Exports::generate_csv($data, $fields);
                    break;
                case 'xls':
                    $content = Beacon_Tax_Exports::generate_xls($data, $fields);
                    break;
                case 'sql':
                    $content = Beacon_Tax_Exports::generate_sql($data, $fields);
                    break;
            }
     ...
     */
    public function beacon_driggs_save_all() {
        check_ajax_referer('beacon_driggs_nonce', 'nonce');
     ...
     */
    public function beacon_driggs_reset() {
        check_ajax_referer('beacon_driggs_nonce', 'nonce');
     ...
     */
    public function beacon_update_site_settings() {
        check_ajax_referer('beacon_driggs_nonce', 'nonce');
