...
3. Updates the widget's content if there's a match</pre>
    ...
    public function snefuru_inject_replex_content() {
        ...
        // Process the elements using the same approach as function_inject_content_2
        $processed_count = 0;
        $elements = self::process_replex_elements($elements, $replacements, $processed_count);
    ...
    /**
     * Process Elementor elements recursively for replex content replacement
     */
        ...
        try {
            if (class_exists('Elementor\Plugin')) {
                ...
                if (isset($elementor_instance->files_manager) && method_exists($elementor_instance->files_manager, 'clear_cache')) {
                    ...
                    wp_send_json_success(array('message' => 'Elementor cache manager not available'));
                }
            } else {
